      ...
      <div className="grid grid-cols-2 gap-3">
        {items.slice(0, 8).map((it, i) => (
          ...
          >
            ...
            <p className="text-sm font-bold leading-tight line-clamp-1" style={{ color: "var(--text)" }}>{it.name || "Item"}</p>
            {it.price ? (
              <p className="text-xs font-mono mt-0.5" style={{ color: "var(--orange)" }}>{it.price}</p>
            ) : null}
          </a>
        ))}
      </div>
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <input type="text" value={config?.headline || ""} onChange={(e) => onChange({ ...config, headline: e.target.value })} maxLength={60} placeholder="My merch"
          className="w-full rounded p-2 text-sm" style={{ background: "var(--surface)", border: "1px solid var(--hairline)", color: "var(--text)" }} />
      </div>
      <div>
        <label className="block text-xs font-black uppercase tracking-widest mb-2" style={{ color: "var(--text-muted)" }}>Items (max 8)</label>
        {items.map((it, i) => (
          <div key={i} className="grid grid-cols-1 md:grid-cols-12 gap-2 mb-2 p-3 rounded" style={{ background: "var(--surface)", border: "1px solid var(--hairline)" }}>
            ...
            <input placeholder="Buy URL" value={it.url || ""} onChange={(e) => updateItem(i, { url: e.target.value })} maxLength={500}
              className="md:col-span-3 rounded p-2 text-sm" style={{ background: "var(--surface-alt)", border: "1px solid var(--hairline)", color: "var(--text)" }} />
        ...
        <button type="button" onClick={addItem} disabled={items.length >= 8} className="text-xs font-black uppercase tracking-widest px-4 py-2 rounded disabled:opacity-40" style={{ background: "var(--orange)", color: "#fff" }}>
