...
// Lead management storage helper (KV-backed; ETag-aware; Worker-compatible)

export class CloudflareLeadStorage {
    constructor(workerUrl, tokenOrGetter = null, opts = {}) {
        ...
        this._opts = {
            ...
            forceAuthForReads: opts.forceAuthForReads ?? true,
        };
    ...
    // ---------------- reads ----------------
    async getAll() {
        const data = await this._getWithEtag("/leads");
    ...
    async update(id, updates, { onProgress } = {}) {
        ...
        this._etags.delete(`${this.workerUrl}/leads`);
        onProgress?.(100);
        return res;
    }
...
export const createLeadStorage = (workerUrl, tokenOrGetter, opts) =>
    new CloudflareLeadStorage(workerUrl, tokenOrGetter, opts);
